...
class NextGreater {
  private stack = new Stack_<number>();

  evaluate(arr: any): number[] { // <-- arr explicitly typed
    const result: any = Array(arr.length).fill(-1);
    this.stack.clear();
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      while (!this.stack.isEmpty()) {
        ...
        if (peekIndex === undefined) break; // safe check

        if (arr[i] > arr[peekIndex]) {
          const index = this.stack.pop()!;
          result[index] = arr[i];
          console.log(
            ` → arr[${index}] = ${arr[index]} → Next Greater = ${arr[i]} → Stack: [${this.stack.toArray()}]`
          );
        } else {
